...
class PairedFusionDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        return x_silhouette, x_skeleton, y, meta
...
def train_one_epoch(
    ...
    for batch in loader:
        if len(batch) == 4:
            x_silhouette, x_skeleton, y, _ = batch
        else:
            x_silhouette, x_skeleton, y = batch
...
def validate_one_epoch(
    ...
    for batch in loader:
        if len(batch) == 4:
            x_silhouette, x_skeleton, y, meta = batch
        else:
            x_silhouette, x_skeleton, y = batch
            meta = None
        ...
        if meta is not None:
            gt_cpu = y.detach().cpu().numpy().tolist()
            pred_cpu = preds.detach().cpu().numpy().tolist()
            for i in range(len(gt_cpu)):
                condition = meta["condition"][i]
                person_id = meta["person_id"][i]
                seq_name = meta["seq_name"][i]
                records.append({
                    "person_id": person_id,
                    "condition": condition,
                    "condition_group": extract_condition_group(condition),
                    "seq_name": seq_name,
                    "gt_label": int(gt_cpu[i]),
                    "pred_label": int(pred_cpu[i]),
                    "correct": int(gt_cpu[i] == pred_cpu[i]),
                })
